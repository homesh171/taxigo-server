...
const sendBookingConfirmation = async (customerEmail, customerName, booking) => {
  try {
    ...
    sendSmtpEmail.subject = `Booking Confirmed - TaxiGo #${booking._id.toString().slice(-6).toUpperCase()}`
    sendSmtpEmail.to = [{ email: customerEmail, name: customerName }]
    sendSmtpEmail.sender = { email: 'londonchauffeur586@gmail.com', name: 'TaxiGo' }
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 32px; background: white;">
          ...
          <div style="background: #fffbeb; border: 1px solid #fde68a; border-radius: 8px; padding: 16px; margin-bottom: 24px;">
            <p style="margin: 0; color: #92400e; font-size: 14px;">⏰ Your driver will be assigned shortly.</p>
          </div>
          ...
          <p style="color: #666; font-size: 13px; text-align: center;">Need help? Email londonchauffeur586@gmail.com</p>
        </div>
...
const sendDriverAssigned = async (customerEmail, customerName, booking, driver) => {
  try {
    const sendSmtpEmail = new SibApiV3Sdk.SendSmtpEmail()
    sendSmtpEmail.subject = `Driver Assigned - TaxiGo #${booking._id.toString().slice(-6).toUpperCase()}`
    sendSmtpEmail.to = [{ email: customerEmail, name: customerName }]
    sendSmtpEmail.sender = { email: 'londonchauffeur586@gmail.com', name: 'TaxiGo' }
    sendSmtpEmail.htmlContent = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 32px; background: white;">
          <p style="font-size: 18px; color: #333;">Hi ${customerName}! 🚗</p>
          <p style="color: #666;">Your driver has been assigned for your upcoming transfer.</p>
          <div style="background: #f9f9f9; border: 1px solid #e5e5e5; border-radius: 10px; padding: 24px; margin: 24px 0;">
            <table style="width: 100%; border-collapse: collapse;">
              <tr style="border-bottom: 1px solid #eee;">
          ...
          <p style="color: #666; font-size: 14px;">Your driver will meet you at arrivals with a <strong>TaxiGo name board</strong>.</p>
        </div>
        <div style="background: #111; padding: 20px; text-align: center; border-radius: 0 0 12px 12px;">
          <p style="color: #666; font-size: 12px; margin: 0;">© ${new Date().getFullYear()} TaxiGo — UK Airport Transfers</p>
        </div>
