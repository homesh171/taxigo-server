...
const sendBookingConfirmation = async (customerEmail, customerName, booking) => {
  try {
    const { data, error } = await resend.emails.send({
      from: 'TaxiGo <onboarding@resend.dev>',
      to: customerEmail,
            ...
            <div style="background: #f9f9f9; border: 1px solid #e5e5e5; border-radius: 10px; padding: 24px; margin: 24px 0;">
              ...
              <table style="width: 100%; border-collapse: collapse;">
                <tr style="border-bottom: 1px solid #eee;">
                  ...
                  <td style="padding: 10px 0; color: #111; font-weight: bold; font-size: 14px; text-align: right;">#${booking._id.toString().slice(-6).toUpperCase()}</td>
                </tr>
                <tr style="border-bottom: 1px solid #eee;">
                  <td style="padding: 10px 0; color: #888; font-size: 14px;">Pickup</td>
                  <td style="padding: 10px 0; color: #111; font-weight: bold; font-size: 14px; text-align: right;">${booking.pickup}</td>
                </tr>
                <tr style="border-bottom: 1px solid #eee;">
                  <td style="padding: 10px 0; color: #888; font-size: 14px;">Drop-off</td>
                  <td style="padding: 10px 0; color: #111; font-weight: bold; font-size: 14px; text-align: right;">${booking.dropoff}</td>
                </tr>
                <tr style="border-bottom: 1px solid #eee;">
                  <td style="padding: 10px 0; color: #888; font-size: 14px;">Date</td>
                  <td style="padding: 10px 0; color: #111; font-weight: bold; font-size: 14px; text-align: right;">${booking.date}</td>
                </tr>
                <tr style="border-bottom: 1px solid #eee;">
                  <td style="padding: 10px 0; color: #888; font-size: 14px;">Time</td>
                  <td style="padding: 10px 0; color: #111; font-weight: bold; font-size: 14px; text-align: right;">${booking.time}</td>
                </tr>
                <tr style="border-bottom: 1px solid #eee;">
                  <td style="padding: 10px 0; color: #888; font-size: 14px;">Vehicle</td>
                  <td style="padding: 10px 0; color: #111; font-weight: bold; font-size: 14px; text-align: right;">${booking.vehicle}</td>
                </tr>
                <tr style="border-bottom: 1px solid #eee;">
                  <td style="padding: 10px 0; color: #888; font-size: 14px;">Passengers</td>
                  <td style="padding: 10px 0; color: #111; font-weight: bold; font-size: 14px; text-align: right;">${booking.passengers}</td>
                </tr>
                ${booking.flight ? `
                <tr style="border-bottom: 1px solid #eee;">
                  <td style="padding: 10px 0; color: #888; font-size: 14px;">Flight</td>
                  <td style="padding: 10px 0; color: #111; font-weight: bold; font-size: 14px; text-align: right;">${booking.flight}</td>
                </tr>` : ''}
                <tr>
    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <div style="padding: 32px; background: white;">
            <p style="font-size: 18px; color: #333;">Hi ${customerName}! 🚗</p>
            <p style="color: #666;">Great news! Your driver has been assigned.</p>
            <div style="background: #f9f9f9; border: 1px solid #e5e5e5; border-radius: 10px; padding: 24px; margin: 24px 0;">
              <table style="width: 100%; border-collapse: collapse;">
                <tr style="border-bottom: 1px solid #eee;">
